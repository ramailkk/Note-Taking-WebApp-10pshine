...
// Consolidated router for /api/note/*.
// Combines what used to be 11 separate files (all, create, dashboard,
// divide, graph, load/[noteId], name/[noteId], notebook/[noteId],
// protect/[noteId], remove/[noteId], save/[noteId]) into a single
// serverless function so the deployment stays under Vercel's Hobby-plan
// function limit.

async function all(req, res, user) {
  if (req.method !== 'GET') {
...
async function load(req, res, user, noteId) {
  if (req.method !== 'GET') {
    return res.status(405).json({ error: 'Method not allowed' });
  }
...
async function name(req, res, user, noteId) {
  if (req.method !== 'PUT') {
    return res.status(405).json({ error: 'Method not allowed' });
  }
...
async function notebook(req, res, user, noteId) {
  if (req.method !== 'PUT') {
    return res.status(405).json({ error: 'Method not allowed' });
  }
...
async function remove(req, res, user, noteId) {
...
async function save(req, res, user, noteId) {
  if (req.method !== 'PUT') {
    return res.status(405).json({ error: 'Method not allowed' });
  }
...
export default async function handler(req, res) {
  ...
  if (first === 'all') return all(req, res, user);
  if (first === 'create') return create(req, res, user);
  if (first === 'dashboard') return dashboard(req, res, user);
  if (first === 'divide') return divide(req, res, user);
  if (first === 'graph') return graph(req, res, user);
  if (first === 'load' && second) return load(req, res, user, second);
  if (first === 'name' && second) return name(req, res, user, second);
  if (first === 'notebook' && second) return notebook(req, res, user, second);
  if (first === 'protect' && second) return protect(req, res, user, second);
  if (first === 'remove' && second) return remove(req, res, user, second);
  if (first === 'save' && second) return save(req, res, user, second);
