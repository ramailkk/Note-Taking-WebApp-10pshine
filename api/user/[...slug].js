...
// Consolidated router for /api/user/*.
// Combines what used to be 4 separate files (info, password, picture,
// profile) into a single serverless function so the deployment stays
// under Vercel's Hobby-plan function limit.
...
export default async function handler(req, res) {
  ...
  if (first === 'info') return info(req, res, user);
  if (first === 'password') return password(req, res, user);
  if (first === 'picture') return picture(req, res, user);
  if (first === 'profile') return profile(req, res, user);
